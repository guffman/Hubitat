 ...
 * v1.6
 * updated 2023-06-12
 * Original driver code by Bryan Copeland (djdizzyd)
 *
 * Modifed by M Chevis - removed a lot of config params I don't want to set via the hub, only at the t-stat
 *                       Added supportedThermostatModes, supported ThermostatFanModes JSON updates per new driver variable format requirements
 */

metadata {
    definition (name: "Advanced GoControl GC-TBZ48", namespace: "guffman", author: "Guffman", importUrl: "" ) {
...
@Field static Map configParams = [
        ...
        9: [input: [name: "configParam9", type: "number", title:"MRT", description: "Minimum Run Time", defaultValue: 9, range: "3..9"], parameterSize: 1],
        ...
        61: [input: [name: "configParam61", type: "number", title:"Fan Purge Heat", description: "seconds", defaultValue: 0, range: "0..90"], parameterSize: 1],
        62: [input: [name: "configParam62", type: "number", title:"Fan Purge Cool", description: "seconds", defaultValue: 0, range: "0..90"], parameterSize: 1],
...
// Pruned to only actionable parameters - M Chevis
@Field static Map configParams = [
        8: [input: [name: "configParam8", type: "number", title:"MOT", description: "Minimum Off Time", defaultValue: 5, range: "5..9"], parameterSize: 1],
        9: [input: [name: "configParam9", type: "number", title:"MRT", description: "Minimum Run Time", defaultValue: 9, range: "3..9"], parameterSize: 1],
        48: [input: [name: "configParam48", type: "number", title:"Internal Sensor Temp Offset", description: "degrees", defaultValue: 0, range: "-7..7"], parameterSize: 1],
...
void initializeVars() {
    ...
    setSupportedThermostatFanModes(JsonOutput.toJson(["auto","on"]))
	  setSupportedThermostatModes(JsonOutput.toJson(["off", "heat", "cool", "auto", "emergency heat"]))    
...
def setSupportedThermostatFanModes(fanModes) {
	if (logEnable) log.debug "setSupportedThermostatFanModes(${fanModes}) was called"
	sendEvent(name: "supportedThermostatFanModes", value: fanModes, descriptionText: "${device.displayName} supportedThermostatFanModes set to ${fanModes}")
}

def setSupportedThermostatModes(modes) {
	if (logEnable) log.debug "setSupportedThermostatModes(${modes}) was called"
	sendEvent(name: "supportedThermostatModes", value: modes, descriptionText: "${device.displayName} supportedThermostatModes set to ${modes}")
}
...
List<hubitat.zwave.Command> pollConfigs() {
    ...
    cmds.add(zwave.configurationV1.configurationGet(parameterNumber: 48))
    return cmds
}
...
void sensorCal(value) {
    if (logEnable) log.debug "sensorCal($value) sent to device"
    List<hubitat.zwave.Command> cmds=[]
...
void zwaveEvent(hubitat.zwave.commands.configurationv1.ConfigurationReport cmd) {
    ...
    if(configParams[cmd.parameterNumber.toInteger()]) {
        ...
        if (cmd.parameterNumber==48) {
            if (logEnable) log.debug "sensorCal($scaledValue) received from device"
